...
- 每个后台业务接口单次响应 **< 2000ms**（AI/地图/短信等第三方及其调用方不计入）。
...
- 分支：`feat/<module>-<slug>`、`fix/<module>-<slug>`、`chore/<slug>`。
- 推送 + 建 MR：`git push -u origin feat/...` → 在 CodeArts 网页新建合并请求，目标 `main`。
- 一个 MR 只做一件事，尽量小、可独立评审。
- MR 标题：`<module>: <做了什么>`；描述按 `docs/merge-request-template.md`。
- `main` 受分支保护：禁直接 push、必须经 MR、≥1 评审 + CI 门禁通过才可合并。
- 改 `contracts/` 需平台核心组评审；合并到 `main` 后由服务器自托管 CI/CD 自动部署。
