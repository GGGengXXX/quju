...
1. **契约即法律。** `contracts/`（DB schema + OpenAPI + 枚举/错误码）是前后端唯一事实来源。**任何对 `contracts/` 的改动都必须**：先写 ADR（`docs/adr/`）→ 单独的 Pull Request → 平台核心组批准（GitHub 分支保护 + 必选评审，见 `docs/codearts-and-cicd.md`）。严禁在 feature 分支里"顺手"改契约。
...
> 代码托管在 **GitHub**。详见 `docs/codearts-and-cicd.md`。
...
- 推送 + 建 PR：`git push -u origin feat/...` → GitHub 新建 Pull Request，**目标 `dev`**。
- 一个 PR 只做一件事，尽量小、可独立评审。PR 标题 `<module>: <做了什么>`，描述按 `docs/merge-request-template.md` 或 `.github/pull_request_template.md`。
- `master`/`dev` 受分支保护：禁直接 push、必须经 PR、≥1 评审 + CI 门禁通过才可合并。
- 改 `contracts/` 需平台核心组评审。**部署**：把 `dev` 合并到 `master` → 自动触发 GitHub Actions 部署。
