...
| [`contracts/`](contracts/) | ⭐ **DB schema + OpenAPI 契约 + 枚举/错误码**，前后端共同的事实来源 | **仅平台核心组**（GitHub 分支保护，见 [codearts-and-cicd.md](docs/codearts-and-cicd.md)） |

> 代码托管：GitHub
> [GGGengXXX/quju](https://github.com/GGGengXXX/quju)
...
5. [`docs/codearts-and-cicd.md`](docs/codearts-and-cicd.md) — GitHub 仓库/推送/PR/GitHub Actions/服务器 CI-CD；[`docs/secrets-and-config.md`](docs/secrets-and-config.md) — 密钥与配置（OSS/服务器，均走环境变量）
...
- **协作**：Monorepo + 服务器上每 feature 一个独立 clone + GitHub Pull Request + **GitHub Actions 自动测试与部署**（合并到 `master` 触发）
