...
| 部署 / CI-CD | 在**单台服务器 `1.92.124.5`** 自托管；合并到 main 自动构建 + 部署 |
| 协作 | 协调方式 = **契约优先 + 垂直切片所有权**；2 人平台核心组守契约 |
| 服务器 | 全员**直接在服务器**开发；每 feature 一分支一 **git worktree**；每人独立 DB schema + 端口段，共用一台 MySQL/Redis |
...
| 服务器上互相踩 | worktree 隔离 + 每人独立 DB/端口 + 文件归属边界 |
