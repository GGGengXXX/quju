...
```
然后在 CodeArts 网页 → 代码托管 → 合并请求 → 新建 MR：源分支 = 你的 feature 分支，目标 = `main`，描述粘贴 `docs/merge-request-template.md`，指定评审人。
...
目标：合并到 `main` 后，**自动在 `1.92.124.5` 上**构建后端 jar、构建前端、重启服务。两种主流做法：

### 方案 A：CodeArts Pipeline + 自定义执行机（华为云原生，推荐用于课程交付）
- 在服务器上安装 **CodeArts 自定义执行机（agent）**，注册到你们的 CodeArts 项目。
- 建一条 **Pipeline**：触发器 = 「合并请求合并 / push 到 main」；阶段 = 代码检查 → 编译构建(Maven + npm) → 部署(在自定义执行机即本服务器上跑 `deploy/deploy.sh`)。
...
3. **建 CodeArts 流水线（方案 A）**：触发=合并到 `main` → 构建(可选) → **部署任务**：目标主机 `1.92.124.5`（SSH，认证用 `codearts_deploy_key` 私钥或服务器密码）→ 执行 `bash /srv/quju/main/deploy/deploy.sh`。
